...
def generate_pdf_from_steps(steps):
    ...
    subheader_style = ParagraphStyle(name='SubHeader', alignment=TA_CENTER, fontSize=25, fontName="Helvetica-Bold")
    normal_center = ParagraphStyle(name='NormalCenter', alignment=TA_CENTER, fontSize=22)
    placement_text_style = ParagraphStyle(name='PlacementText', alignment=TA_CENTER, fontSize=20)
    component_name_style = ParagraphStyle(name='ComponentName', alignment=TA_CENTER, fontSize=22)
    weight_style = ParagraphStyle(name='Weight', alignment=TA_CENTER, fontSize=90)

    for step in steps:
        # Header table
        col_data = [
            ["<b>COMPONENT TYPE</b>", "<b>STEP</b>", "<b>MEAL CODE</b>"],
            ["<b>(TIPO DE COMPONENTE)</b>", "<b>(PASO)</b>", "<b>(CÓDIGO DE COMIDA)</b>"]
        ]
