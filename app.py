...
def generate_pdf_from_steps(steps):
    ...
    for step in steps:
        ...
        std_table = Table([["STANDARD (SIZE)"], [Paragraph(step["Standard Size"], ParagraphStyle(name="BoldNumber", fontName="Helvetica-Bold", fontSize=36, alignment=TA_CENTER))]], colWidths=[6.6 * inch])
        ...
        large_table = Table([["LARGE (SIZE)"], [Paragraph(step["Large Size"], ParagraphStyle(name="BoldNumber", fontName="Helvetica-Bold", fontSize=36, alignment=TA_CENTER))]], colWidths=[6.6 * inch])
